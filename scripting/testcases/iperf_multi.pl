...
=Usage:
 ...
 -fd		=> Flow rate policy will be created on the specified direction. Default "any". (inbound or outbound)
 -pd		=> Class pipe will be created on the specified direction. Default "any". (inbound or outbound)
...
my $ip_ver_4 = "ipv4";
my $ip_ver_6 = "ipv6";
my $in_direction = " inbound ";
my $out_direction = " outbound ";
...
GetOptions("appl_name|app|a=s" => \$appl_name, "input-file|test-list|i=s" => \$input_file, "usage|test-type|u=s" => \$traffic_type,
			"shaping_off|both|d=s" => \$test_times, "protocol|ip=s" => \$ip_protocol, "flowdirection|fd=s" => \$fl_direction, "pipedirection|pd=s" => \$cl_direction);
...
# Direction Checking for Flowrate policy
if(lc($fl_direction) eq "inbound")
{
	$fl_direction = $in_direction;
}elsif(lc($fl_direction) eq "outbound")
{
	$fl_direction = $out_direction;
}else
{
	$fl_direction = " ";
}

# Direction Checking for class pipe
if(lc($cl_direction) eq "inbound")
{
	$cl_direction = $in_direction;
}elsif(lc($cl_direction) eq "outbound")
{
	$cl_direction = $out_direction;
}else
{
	$cl_direction = " ";
}
...
}else
{
	...
	for(my $j=0; $j <= $#udp_class_packet_size; $j++)
	{
		if(@udp_class_packet_size[$j] < 22)
		{
			...
			print_log($LOG, "UDP packet size should not be less than 63 bytes. Packet size set to Default value for @class_names[$j].");
		}
	}
}
...
for($i = 0 ; $i <= $#class_names ; $i++)
{
	...
	execute(create_policy($appliance, @flowrate_policies[$i], "flowrate" . $fl_direction ."guaranteed 10"));
...
for my $hash (@all_test_inputs) 
{
	...
	# The hash reference variable "$hash" contains values for a the test
	# Load pipe, priority and flowrate of the classes for a test
	...
	{
		# If we give number of flows to '0' that traffic won't run on that class
		if(@no_of_flows[$i] > 0)
		{
			# Calculating the initial pipe min, burst-max and flowrate min, burst-max
			@min_bw[$i] = int(to_percentage($wan_speed, @pipemin_percentage[$i]));
			@burstmax[$i] = int(to_percentage($wan_speed, @pipemax_percentage[$i]));
			@flowrate_min_bw[$i] = int(to_percentage(@min_bw[$i], @flowrate_min_percentage[$i]));
			@flowrate_burstmax[$i] = int(to_percentage(@min_bw[$i], @flowrate_max_percentage[$i]));
			
			# Reseting pipe
			execute(set_pipe($appliance, 0, 0, @class_names[$i]));
			execute(set_pipe($appliance, @min_bw[$i], @burstmax[$i], @class_names[$i], $cl_direction));
			...
			}elsif(@flowrate_min_bw[$i] > 0 && @flowrate_burstmax[$i] == 0)
			{
				...
				execute(change_policy($appliance, @flowrate_policies[$i], "flowrate" . $fl_direction ."guaranteed @flowrate_min_bw[$i]"));
			...
			}else
			{
				...
				my $option = "flowrate" . $fl_direction ."guaranteed @flowrate_min_bw[$i] burst: @flowrate_burstmax[$i] priority: @priorities[$i]";
...
my $body_text = "Please find attached output and log files for the build sanity test results. Flow direction $fl_direction. Pipe direction $cl_direction.";
...
sub collect_stats_all_classes
{
	...
	foreach(@classes)
	{
		...
		$report = $report . fs($_)."|$stats{\"cs_in_tx_kbps\"}\t|$stats{\"cs_out_tx_kbps\"}\t |" . ($stats{"cs_out_flow_count"} + $stats{"cs_in_flow_count"}). "\n";
	}
	$report = $report . "----------------------------------------------\n";
	$report = $report . "Interface\t|RateIN\t|RateOUT |Flows\n";
	$report = $report . fs($if_stats{'ls_link'}). "|$if_stats{'ls_int_tx_kbps'}\t|$if_stats{'ls_ext_tx_kbps'}\t |" . ($if_stats{'ls_ext_flow_count'} + $if_stats{'ls_int_flow_count'}) . "\n";
	$report = $report . "----------------------------------------------\n\n\n";
...
sub check_stats
{
	...
	my @flowrate_burstmax = ();			# values in Kbps
	my @priorities = ();				# priority values
	my @udp_traffic_bandwidth_percentage = ();	# values in Kbps
	my @no_of_flows = ();				# number of flows
	...
	foreach (sort keys %{$hash})
	{
		if($_ ne "wan_speed" && $_ ne "test_duration" && $_ ne "test_id")
		{
			push @min_bw ,int(to_percentage($wan_speed, @{$hash->{$_}}[0]));
			push @burstmax ,int(to_percentage($wan_speed, @{$hash->{$_}}[1]));
			push @flowrate_min_bw ,int(to_percentage(@min_bw[$i], @{$hash->{$_}}[2]));
			push @flowrate_burstmax, int(to_percentage(@min_bw[$i], @{$hash->{$_}}[3]));
	...
	my $wan_speed = $hash->{'wan_speed'};
	my $test_interval = $hash->{'test_duration'};

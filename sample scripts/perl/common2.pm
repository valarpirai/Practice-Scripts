...
# Global variables
our (%cfg, %global_cfg, $SUCCESS, $FAIL, $YES, $NO, $LOG, $OUT, $ANY, $LINK0, $LINK1);
%cfg = ();
%global_cfg = ();
$SUCCESS = "success";
$FAIL = "fail";
...
=cut
	...
	$lic_ref->{'customer_id'} = trim(@first_line[0]);
	$lic_ref->{'key'} = trim(@first_line[1]).trim(@second_line[1]);
	$lic_ref->{'mac_addr'} = trim(@first_line[2]);
	...
	$lic_ref->{'link_speed_kbps'} = "$link_speed";
	$lic_ref->{'links_count'} = int(trim(@first_line[4]));
	$lic_ref->{'dual_link'} = $NO;
	if(int(trim(@first_line[4])) > 1)
	{
		$lic_ref->{'dual_link'} = $YES;
	}
	if(trim(@first_line[5]) eq "Monitoring,")
	{
		$lic_ref->{'enabled_monitoring'} = $YES;
	}else
	{
		$lic_ref->{'enabled_monitoring'} = $NO;
	}
		
	if(trim(@second_line[5]) eq "TM")
	{
		$lic_ref->{'enabled_traffic_management'} = $YES;
	}else
	{
		$lic_ref->{'enabled_traffic_management'} = $NO;
	}
		
	$lic_ref->{'max_classes'} = trim(@first_line[6]);
	$lic_ref->{'max_flows'} = trim(@first_line[7]);
	$lic_ref->{'max_policies'} = trim(@first_line[8]);
	$lic_ref->{'expiry_date'} = trim(@first_line[9]);
...
sub create_class
{
	my($session, $classname, $linkname, $pattname, $refclassname) = @_;
	...
	my $cmd = "app\nnew class $classname $linkname any Custom pattern: $pattname $refclassname";
	trim($cmd);
	print $session "$cmd\n";

	unless ($session->expect($timeout, "New class $classname created successfully"))
	{
		my $out = $session->before();
...
sub get_link_stats
{
	...
	print_log($LOG, "Link stats Command : $cmd");
	print $session "$cmd";
	
	# Expecting the starting header
	unless($session->expect($shorttimeout, "-re", "show stats interface $link any\n-+\n\|Name.*Flows.*Rx Bytes.*Tx Bytes.*Rx Kbps.*\n-.*"))
	{
...
sub get_software_version
{
	...
	# Expecting the starting header
	unless($session->expect($shorttimeout, "-re", "show img.*\n-+\n\|Parameter.*Value.*\n-.*"))
	{
		return "Not able to get the $cmd output";
	}
...
sub set_pipe
{
	...
	my $cmd = "policy\nnew pipe min $pipe_min burstmax: $pipe_max $class_name\n";
	print $session "$cmd";
	my $match = "Added new pipe min:" . $pipe_min . "kbps, max:" . $pipe_max . "kbps for class $class_name (any)successfully";
	unless($session->expect(1, $match))
	{
		return "Failed to assign pipe min " . $pipe_min . "kbps, max:" . $pipe_max . "kbps for $class_name.\n";
	}
	return $SUCCESS;
}

...
print $appliance "assign policy $EC_priority_policy_name $EC_class_name\n";
my $match = "Assigned policy $EC_priority_policy_name to $EC_class_name successfully";
unless($appliance->expect(1, $match))
{
	execute("Failed to assign policy $EC_priority_policy_name to $EC_class_name.\n");
}
...
=cut	
	...
	#print $appliance "policy\n new pipe min $EC_pipe_min $EC_class_name\n";
	#my $match = "Added new pipe min:" . @str[0] . "kbps, max:kbps for class $EC_class_name (any)successfully";
	
	print $appliance "policy\n\rnew pipe min $EC_pipe_min burstmax: $EC_pipe_max $EC_class_name\n";
	my $match = "Added new pipe min:" . $EC_pipe_min . "kbps, max:" . $EC_pipe_max . "kbps for class $EC_class_name (any)successfully";
	unless($appliance->expect(1, $match))
	{
		execute("Failed to assign pipe min for $EC_class_name.\n");
		exit;
	}
	
	
	print $appliance "set policy $EC_priority_policy_name priority $EC_priority\n";
	my $match = "Updated policy $EC_priority_policy_name successfully";
	unless($appliance->expect(1, $match))
	{
		execute("Failed to change policy $EC_priority_policy_name.\n");
	}
	
	print_log($ANY, "Test starting with the Config.\nOther Traffic - no_of_flows: $OT_no_of_flows, class_rate :$EC_total_rate\nTraffic in class no_of_flows: $EC_no_of_flows, class_rate :$EC_total_rate");
...
sub use_this_configuration
{
	...
	start_iperf_clients($client, \$pid, $cfg{'server_loc_eth_ip'}, $OT_no_of_flows, $total_duration, $total_duration, $OT_flow_rate, " -p 5001");	# Class Traffic
	...
	$check_1 = ($run_stats{"in_tx_Kbps"} >= $display_bw);
	$check_2 = ($run_stats{"out_tx_Kbps"} >= $display_bw);

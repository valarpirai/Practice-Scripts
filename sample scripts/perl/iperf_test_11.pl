...
=cut	
	...
	print $appliance "policy\n\rnew pipe min $OT_pipe_min burstmax: $OT_pipe_max $OT_class_name\n";
	my $match = "Added new pipe min:" . $OT_pipe_min . "kbps, max:" . $OT_pipe_max . "kbps for class $OT_class_name (any)successfully";
	unless($appliance->expect(1, $match))
	{
		execute("Failed to assign pipe min for $OT_class_name.\n");
		exit;
	}
	
	
	print $appliance "set policy $OT_priority_policy_name priority $OT_priority\n\n";
	my $match = "Updated policy $OT_priority_policy_name successfully";
	unless($appliance->expect(1, $match))
	{
		execute("Failed to change policy $OT_priority_policy_name.\n");
	}
